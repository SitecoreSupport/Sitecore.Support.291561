...
namespace Sitecore.Support.Resources.Media
{
    ...
    {
        private MediaCreator medcreator = null;
        ...
        {
            this.medcreator = new MediaCreator();
        }
                ...
                using (ZipReader zipReader = new ZipReader(text))
                {
                    foreach (ZipEntry current in zipReader.Entries)
                    {
                        if (!current.IsDirectory)
                        {
                            ...
                            mediaUploadResult.Item = this.medcreator.CreateFromStream(stream, mediaUploadResult.Path, mediaCreatorOptions);
                        }
                    }
                }
            }
